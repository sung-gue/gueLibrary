...
public class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    protected boolean _isUserSetMaxZoom;
     ...
     * @param reset  - if true the image bounds will be recreated, otherwise the old {@link Matrix} is used to display the new bitmap
    ...
    protected void _setImageDrawable(final Drawable drawable, final boolean reset, final Matrix initial_matrix, final float maxZoom) {
        ...
        if (maxZoom < 1) {
            if (!_isUserSetMaxZoom)
                mMaxZoom = maxZoom();    // 2013.7.5/gue : mMaxZoom 이 수동으로 설정된 경우 적정값으로 변경하지 않음
//			mMaxZoom = maxZoom();
...
//		float scale = Math.min( widthScale, heightScale );
    ...
    protected void zoomTo(float scale, float centerX, float centerY) {
        ...
        Log.d(LOG_TAG, "zoomTo: " + scale + ", center: " + centerX + "x" + centerY + ", maxZoom: " + mMaxZoom);
