...
//    	_semaphore = new Semaphore(6);
	...
	public final synchronized void getQueue(String tag, String msg){
		Log.i(TAG, String.format("(%s) 1-1. current queue count : %d/%s", tag, _loaderQueue, msg ));
		...
		_loaderQueue++;
		Log.i(TAG, String.format("(%s) 1-2. add queue count : %d/%s", tag, _loaderQueue, msg ));
		
		/*Log.i(TAG, String.format("(%s)/%s 1-1. current queue count : %d/%s", Thread.currentThread().getName(), tag, 6 -  _semaphore.availablePermits(), msg ));
		try {
			_semaphore.acquire();
		} catch (InterruptedException e) {
			Log.w(TAG, "InterruptedException : " + msg, e);
		}
		Log.i(TAG, String.format("(%s)/%s 1-2. add queue count : %d/%s", Thread.currentThread().getName(), tag,  6 - _semaphore.availablePermits(), msg ));
//		_semaphore.release();
//		Log.i(TAG, String.format("(%s)/%s 2. remove queue count : %d/%s", Thread.currentThread().getName(), tag, 6 - _semaphore.availablePermits(), msg ));*/		
    }
    ...
    public final synchronized void removeQueue(String tag, String msg){
    	...
    	Log.i(TAG, String.format("(%s) 2. remove queue count : %d/%s", tag, _loaderQueue, msg ));
    	
    	/*_semaphore.release();
    	if (DEBUG) Log.i(TAG, String.format("(%s)/%s 2. remove queue count : %d/%s", Thread.currentThread().getName(), tag, _loaderQueueMax - _semaphore.availablePermits(), msg ));*/
    }
}